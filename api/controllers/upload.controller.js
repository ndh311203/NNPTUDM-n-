...
exports.uploadMultipleFiles = (req, res) => {
    try {
        if (!req.files || req.files.length === 0) {
            return res.status(400).json({ success: false, message: 'Vui lòng chọn file đính kèm' });
        }
...
exports.deleteUploadedFile = (req, res) => {
    try {
        ...
        if (!deleted) {
            return res.status(404).json({ success: false, message: 'Không tìm thấy file để xóa hoặc lỗi' });
        }
