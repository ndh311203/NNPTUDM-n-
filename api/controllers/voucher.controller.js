...
exports.updateVoucher = async (req, res) => {
    try {
        ...
        if (error.code === 11000) {
            return res.status(400).json({ success: false, message: 'Mã voucher đã được sử dụng' });
        }
        res.status(500).json({ success: false, message: error.message });
    }
};
...
exports.validateVoucher = async (req, res) => {
    try {
        ...
        if (!voucher.trangThai) {
            return res.status(400).json({ success: false, message: 'Voucher đã bị vô hiệu hóa' });
        }

        const now = new Date();
        if (voucher.ngayBatDau > now) {
            return res.status(400).json({ success: false, message: 'Voucher chưa tới thời gian sử dụng' });
        }
        ...
        if (orderValue !== undefined && voucher.giaTriDonHangToiThieu > orderValue) {
