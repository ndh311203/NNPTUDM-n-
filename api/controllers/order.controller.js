...
exports.createOrder = async (req, res) => {
    ...
    try {
        const { items, khachHangId, voucherSuDung, diaChiGiaoHang, tongTien } = req.body;
        
        // Cập nhật tồn kho (Stock quantity update transaction)
        for (const item of items) {
            const sanPham = await SanPham.findById(item.sanPhamId).session(session);
            if (!sanPham || sanPham.soLuongTon < item.soLuong) {
                throw new Error(`Sản phẩm ${sanPham ? sanPham.tenSanPham : item.sanPhamId} không đủ số lượng tồn kho`);
            }
...
exports.updateOrder = async (req, res) => {
    try {
        const order = await ShopOrder.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
        if (!order) return res.status(404).json({ success: false, message: 'Đơn hàng không tồn tại' });
